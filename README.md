...
2. **Paste a public GitHub repo link** — e.g. `https://github.com/owner/repo`. Only its `.py` files are fetched — via the GitHub API, one file at a time, into a temp directory ([`fetcher.py`](fetcher.py)) — then scanned and deleted; local and deployed runs behave identically. See [Fetching a repo](#fetching-a-repo).
...
The taint-guided call-graph walk is bounded on purpose: **3 hops deep, 12 helpers wide** per finding, with a visited-set cycle guard so it always terminates (no infinite loops, even on mutually-recursive code). A fix buried deeper than 3 hops, or past the 12th helper, won't be seen — and again the finding is **kept**. The taint tracking is **conservative**: it over-includes helpers rather than risk missing a tainted path, so a bundle can carry a helper or two that turn out irrelevant.

The walk seeds taint from a function's **parameters** *and* from a short list of known input **sources** — `input()`, `os.environ`/`os.getenv`, `sys.argv` — so a handler that takes **no parameters** but reads from one of those still gets its chain walked and its finding cleared. What it deliberately does **not** treat as a source is an **arbitrary module global**: a global can hold anything, so tainting every global read would over-drop — the unsafe direction. A finding like that is **kept**, and only the **agent** (whose `read_function` isn't taint-gated) may choose to open its helper. Extending the source list to framework request objects (`request.args`, …) is the natural next step (see [What's next](#whats-next)).
...
- **Taint from framework request objects** — the source list already seeds from `input()`, `os.environ`/`os.getenv`, and `sys.argv` (so a no-parameter handler that reads one of those is cleared deterministically); extending it to `request.args`/`request.form` and, carefully, request-like globals would clear the remaining no-parameter case too — a handler that reads its input from a module global, which the walk can't yet seed.
